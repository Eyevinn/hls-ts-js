...
class NalUParser {
  ...
  sps() {
    ...
    const profileConstraintsFlags = rbspData.readFlags(6);
    // Skip reserved bits
    rbspData.skipBits(2);
        ...
        profileIdc === 138 || profileIdc === 139 || profileIdc === 134 || profileIdc === 135) 
    {
      ...
      let seperateColourPlaneFlag;
      if (nalUnitSPS.chromaFormatIdc === 3) {
        const flags = rbspData.readFlags(1);
        seperateColourPlaneFlag = flags[0];
      }
      const bitDepthLumaMinus8 = rbspData.readUE();
      const bitDepthChromaMinus8 = rbspData.readUE();
      const flags = rbspData.readFlags(2);
      const qpPrimeYZeroTransformBypassFlag = flags[0];
      const seqScalingMatrixPresentFlag = flags[1];
      if (seqScalingMatrixPresentFlag) {
        const seqScalingListPresentFlag = rbspData.readFlags(chromaFormatIdc != 3 ? 8 : 12);
      }
    }
    const log2MaxFrameNumMinus4 = rbspData.readUE();
    nalUnitSPS.picOrderCntType = rbspData.readUE();
    if (nalUnitSPS.picOrderCntType === 0) {
      const log2MaxPicOrderCntLsbMinus4 = rbspData.readUE();
    } else if (nalUnitSPS.picOrderCntType === 1) {
      const deltaPicOrderAlwaysZeroFlag = rbspData.readFlags(1)[0];
      const offsetForNonRefPic = rbspData.readSE();
      const offsetForTopToBottomField = rbspData.readSE();
      const numRefFrameInPicOrderCntCycle = rbspData.readUE();
      let offsetForRefFrame = [];
      for (let i = 0; i < numRefFrameInPicOrderCntCycle; i++) {
        offsetForRefFrame.push(rbspData.readSE());
      }
    }
    nalUnitSPS.maxNumRefFrames = rbspData.readUE();
    const gapsInFrameNumValueAllowedFlag = rbspData.readFlags(1)[0];

...
const TSParser = function constructor() {
  ...
  this.pmt;
  this.media = {
    avc: this._initStream(),
    aac: this._initStream(),
    scte35: this._initStream(),
  };
    ...
    if (chunk[pos] === SYNC_BYTE) {
      ...
      if (this.pat && this.pat.pmtId && packet.pid === this.pat.pmtId) {
        ...
        if (packet.pid === this.pmt.avc) {
          if (packet.pusi) {
            if (this.media.avc.size > 0) {
              const pes = this._parsePES(this.media.avc);
              if (pes) {
                //log.debug(`PES(AVC): PTS:${pes.pts}, ${pes.payload.length} bytes`);
                packet.pts = pes.pts;
                packet.dts = pes.dts;
                this.media.avc.payload = pes.payload;
              }
            }
          }
          this.media.avc.data.push(chunk.subarray(offset, pos + PACKET_SIZE));
          this.media.avc.size += pos + PACKET_SIZE - offset;
        } else if (packet.pid === this.pmt.aac) {
          if (packet.pusi) {
            if (this.media.aac.size > 0) {
              const pes = this._parsePES(this.media.aac);
              if (pes) {
                //log.debug(`PES(AAC): PTS:${pes.pts}, ${pes.payload.length} bytes`);
                packet.pts = pes.pts;
                packet.dts = pes.dts;
                this.media.aac.payload = pes.payload;
              }
            }
          }
          this.media.aac.data.push(chunk.subarray(offset, pos + PACKET_SIZE));
          this.media.aac.size += pos + PACKET_SIZE - offset;
        } else if (packet.pid === this.pmt.scte35) {
          if (packet.pusi) {
            if (this.media.scte35.size > 0) {
              const pes = this._parsePES(this.media.aac);
              if (pes) {
                packet.pts = pes.pts;
                packet.dts = pes.dts;
          ...
          this.media.scte35.data.push(chunk.subarray(offset, pos + PACKET_SIZE));
          this.media.scte35.size += pos + PACKET_SIZE - offset;
        }
      } else {
        switch(packet.pid) {
          case PAT_ID:
...
TSParser.prototype._initStream = function _initStream() {
  return {
    data: [],
    payload: undefined,
    size: 0,
...
TSParser.prototype._parsePES = function _parsePES(stream) {
  ...
  if (stream.data[0].length < MIN_PES_HDR_SIZE) {
    ...
    throw new Error("Insufficient data to parse PES header");
  }
  const pes = new PES();
